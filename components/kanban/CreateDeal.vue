...
interface IDealForm extends Pick<IDeal, name | price> {
    customer: {
        name: string;
        email: string;
    };
...
const { handleSubmit, defineField, handleReset } = useForm<IDealForm>({
    initialValues: {
		status: props.status,
	},
})
...
const { mutate, isPending } = useMutation({
    ...
    mutationFn: (data: IDealForm) => db.createDocument(DB_ID, COLLECTION_DEALS, uuid(), data),
    onSuccess() {
...
<template>
    ...
    <form v-if="isOpenForm" @submit="onSubmit" class="form">
        <UiInput placeholder="Наименование" v-model="name" v-bind="nameAttrs" type="text" class="input"
            :ref="dealNameRef" />
        <UiInput placeholder="Сумма" v-model="price" v-bind="priceAttrs" type="text" class="input" :ref="dealAmountRef" />
